...
class DeployerAgent:
    ...
    def deploy_project(self, project: GeneratedProject, project_dir: str) -> AgentResponse:
        ...
        try:
            ...
            # If no platform succeeded, provide graceful fallback with manual deployment instructions
            if not deployment_url:
                # Check if npm is available
                npm_available = self._check_npm_available()
                
                if not npm_available:
                    # npm not available - provide manual deployment instructions
                    manual_instructions = self._generate_manual_deployment_instructions(project_dir)
                    
                    # Log that automatic deployment was skipped
                    memory.add_entry(
                        agent='deployer',
                        action='automatic_deployment_skipped',
                        data={
                            'reason': 'npm_not_available',
                            'project_location': project_dir,
                            'manual_deployment_available': True
                        },
                        tags=['deployment', 'manual', 'npm_missing'],
                        importance=0.8
                    )
                    
                    # Return success with manual deployment instructions
                ...
                # npm is available but deployment still failed
    ...
    def _generate_manual_deployment_instructions(self, project_dir: str) -> str:
        """
        Generate manual deployment instructions when automatic deployment is not available
        
        Args:
            project_dir: Path to the generated project directory
            
        Returns:
            Formatted instructions for manual deployment
        """
        instructions = f"""
...
3. Navigate to project: cd {project_dir}
...
1. Install Node.js from: https://nodejs.org/
2. Navigate to project: cd {project_dir}
